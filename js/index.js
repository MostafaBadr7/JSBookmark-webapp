...
*/
var regExURL =
  /^((https?)|(ftp)|(file)):\/\/\/?((([A-Za-z]:){1}|([a-zA-Z:-]{1,100}\d*\.\w{1,50}\.?[a-zA-Z]{2,10}))|((\d{1,3}\.){3}\d{1,3}(:(?=\d{1,4}))?))[\w-@%+.~#\?&\/=_]*$/;
...
if (bookmarksInStorage != null) {
  crudBookmarksArray = bookmarksInStorage;
}
...
function edit(editID) {
  ...
  if (crudBookmarksArray[editID - 1].websiteName != Bookmark.websiteName) {
    //If name chngd chck URL
    if (crudBookmarksArray[editID - 1].websiteURL != Bookmark.websiteURL) {
      //If URL changed validate it -- else update the name
      if (validateBookmark(Bookmark) === true) {
        crudBookmarksArray.splice(editID - 1, 1, Bookmark); //editID sent on update bookmark button click / (-1) to get the index
        localStorage.setItem(
          "crud-Bookmarks",
          JSON.stringify(crudBookmarksArray)
        );
        displayBookmarks();
        resetForm();
...
searchBtn.addEventListener("click", () => {
  ...
  for (var i = 1; i <= crudBookmarksArray.length; i++) {
    console.log(crudBookmarksArray[i - 1].websiteName);
    if (
      crudBookmarksArray[i - 1].websiteName.indexOf(searchElement.value) > -1 ||
      crudBookmarksArray[i - 1].websiteURL.indexOf(searchElement.value) > -1 ||
      crudBookmarksArray[i - 1].websiteNote.indexOf(searchElement.value) > -1
    ) {
      bookmarksHtmlLines += `
                    <tr class="w-100  ">
                        <th  scope="row">${i}</th>
...
function validateBookmark(Bookmark) {
  // Validate URL
  if (regExURL.test(Bookmark.websiteURL) === false) {
    //chck if URL valid
  ...
  if (!Bookmark.websiteURL | !Bookmark.websiteName) {
    //If input url or name empty
